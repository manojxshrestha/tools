...
check_tools() {
    tools=("gau" "waymore" "urlfinder" "anew" "pm")
...
if [[ "$1" == "-d" ]]; then
    ...
    cat $base_dir/all_urls.txt | grep -av "wp-" | anew $base_dir/all_unique_urls.txt

    pm -f $base_dir/all_unique_urls.txt -p /opt/Pattern-Matching/patterns/db_params.txt -c -i -r "BXSS" -o $base_dir/get_params_bxss.txt
